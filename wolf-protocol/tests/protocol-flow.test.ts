...
// Covers all 8 required test cases from CLAUDE_PROMPT.md
...
// Known token addresses with deterministic scores (pre-calculated)
const HIGH_SCORE_TOKEN = "So11111111111111111111111111111111111111112"; // score ~76+
const LOW_SCORE_TOKEN  = "rug1111111111111111111111111111111111111111"; // score ~20-
...
describe("5. Rejects execution if token score is below threshold", () => {
  it("rejects when minSignalScore is set higher than token score", () => {
    ...
    const result = createExecutionRequest({
      ...
      actionType: "monitor",
      targetToken: HIGH_SCORE_TOKEN,
      minSignalScore: 999, // impossible threshold
      requireProtectedRoute: false,
    });
...
describe("6. Rejects swap without maxSlippageBps", () => {
  it("rejects swap action when maxSlippageBps is missing", () => {
    ...
    const result = createExecutionRequest({
      ...
      actionType: "swap",
      targetToken: HIGH_SCORE_TOKEN,
      minSignalScore: 1, // accept any score
      requireProtectedRoute: false,
      // maxSlippageBps intentionally omitted
    });
